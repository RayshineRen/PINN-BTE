...
class PINN:
    def __init__(self, layers, L, D, S, r_f, r_b, phi_b):
        ...
        self.scheduler = StepLR(self.adam_optimizer, step_size=1000, gamma=0.99)
    ...
    def f(self, r):
        ...
        return phi_rr + 2.0 * phi_r / r - phi / self.L**2

    def bc(self, r):
        phi = self.dnn(r)
        phi_r = torch.autograd.grad(phi.sum(), r, create_graph=True)[0]
        return -4 * np.pi * self.D * r**2 * phi_r - self.S
...
with open("Exact.txt", "w") as f:
    for i in range(Exact.shape[0]):
        f.write(str(r[i, 0]) + " " + str(Exact[i, 0]) + "\n")
